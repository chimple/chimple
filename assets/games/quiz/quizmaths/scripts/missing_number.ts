...
export class MissingNumber extends cc.Component {
  ...
  protected onLoad(): void {
    ...
    this.node.on(MATH_MATCH, (event) => {
      ...
      if (
        Array.from(this.correctDrops.keys()).length ===
        this.individualNumbers.length
      ) {
        const allCorrect = Array.from(this.correctDrops.values()).every(
          (n) => n === true
        );
        if (allCorrect) {
  ...
  generateChoices() {
    ...
    if (values && values.length === 2) {
      ...
      if (
        this.quizConfig.order === SORT_ASC ||
        this.quizConfig.order === SORT_ASCENDING
      ) {
  ...
  renderDropPanel() {
    ...
    this.choices.forEach((c, i) => {
      if (i == this.answerIndex - 1) {
        ...
        QuizHelper.createTextLabelWithContent(
          this.quizConfig,
          dropPanel,
          c,
          this.label,
