...
export class Grid extends Game {
    ...
    private processConfiguration(data: any[] = []): GridConfig | null {
        ...
        let [level, workSheet, problemNo, verticalProblemStr, horizontalProblemStr, aHorizontalProblemStr] = configurations;
        let ahorizontalProblemStr = !aHorizontalProblemStr ? horizontalProblemStr : aHorizontalProblemStr;
        return {
            level,
            workSheet,
            problemNo,
    ...
    private buildGround(): void {
        ...
        const lh: string = this.currentConfig.horizontalProblem.reduce(function (a, b) {
            return a.length > b.length ? a : b;
        });

        const lv: string = this.currentConfig.verticalProblem.reduce(function (a, b) {
            return a.length > b.length ? a : b;
        });
        ...
        let groundWidth: number = 5 * V_MARGIN + Grid._maxNodeWidth + (horizontalWordsCount * Grid._maxNodeWidth);
        let groundHeight: number = 8 * V_MARGIN + Grid._maxNodeHeight + (verticalWordsCount * Grid._maxNodeHeight);
    ...
    @catchError()
    private renderWordMatrix(choices: GridElement[], prefab: cc.Prefab, blockType: BLOCK_TYPE) {
        const length: number = choices.length;
        choices.forEach(
            (choice: GridElement, index: number) => {
    ...
    @catchError()
    private buildAnswersAndPlaceHolders(horizontalConfigs: string[], verticalConfigs: string[], aHorizontalConfigs: string[]): void {
    ...
    @catchError()
    private computeAnswers(horizontalConfigs: string[], verticalConfigs: string[], aHorizontalConfigs: string[]): GridElement[] {
        console.log('horizontalConfigs', horizontalConfigs);
        console.log('aHorizontalConfigs', aHorizontalConfigs);
        const wordMatrixElements: GridElement[] =
            this.flattenDeep<string[]>(verticalConfigs.map((vText: string, y: number) => horizontalConfigs.map((hText: string, x: number) => {
                return {
                    text: vText + aHorizontalConfigs[x],
                    placeHolderPosition: new Vec2(Grid._horizontalPositions[x], Grid._verticalPositions[y]),
    ...
    public wordMatched(word: string): void {
        ...
        this._remainingAnswers = this._remainingAnswers.filter((a: GridElement) => a.text !== word);
        if (this._remainingAnswers.length <= 0) {
            this.node.emit('nextProblem');
        } else {
            if (this._matchedCounterInCurrentRun === this.currentConfig.horizontalProblem.length) {
                this.renderWordMatrix(this.slices(this._remainingAnswers, this.currentConfig.horizontalProblem.length), this.answerBlockPrefab, BLOCK_TYPE.ANSWER);
            }
        }
    }
}
