...
export default class AnswerBlock extends CommonBlock {
    ...
    protected start(): void {
        ...
        this.pairingPlaceHolderBlock = ground.getChildByName(matchedPlaceHolderName).getComponent(WordBlock);
        ...
        this.matchRect = cc.Rect.fromMinMax
            (
                ...
                cc.v2(
                    pairingPlaceHolderBlockBox.x + pairingPlaceHolderBlockBox.width - MIN_MATCH,
                    pairingPlaceHolderBlockBox.y + pairingPlaceHolderBlockBox.height - MIN_MATCH
                )
            );
    ...
    public renderAnswerHolder(renderParams: RenderParams): void {
        ...
        if (!!renderParams.combinedQAndA) {
            ...
            renderParams.combinedQAndA.split('-')
                .map((s: string) => {
    ...
    onTouchStart(touch: cc.Touch) {
        const nPos: Vec2 = this.node.getParent().convertToNodeSpaceAR(touch.getLocation());
        this._startPos = nPos;
        if (!this.match) {
            this.moved = false;
            new cc.Tween().target(this.node)
                .to(0.1, { scaleX: this.checkRTLAndScale(1.1), scaleY: 1.1 * SCALE }, { progress: null, easing: 'sineOut' })
                .call(() => {
                    this.speak();
                })
    ...
    onTouchMove(touch: cc.Touch) {
        this.moved = true;
        const delta = new cc.Vec2(1 / MATRIX_CONTAINER_SCALE * touch.getDelta().x, 1 / MATRIX_CONTAINER_SCALE * touch.getDelta().y);
        ...
        if (this.node.getBoundingBox().intersects(this.matchRect)) {
            ...
            questions.forEach((q: string) => {
                ...
                const questionBlock: QuestionBlock = ground.getChildByName(q).getComponent(QuestionBlock);
        ...
        new cc.Tween().target(this.node)
            ...
            .to(0.15, { scaleX: this.checkRTLAndScale(1), scaleY: SCALE }, { progress: null, easing: 'sineOut' })
            .start();
    }

    @catchError()
    onTouchEnd(touch: cc.Touch) {
        const ePos: Vec2 = this.node.getParent().convertToNodeSpaceAR(touch.getLocation());
    ...
    private matchFound() {
        ...
        this.moveToPos(this.finishPosition)
            .call(() => {
                ...
                this.sparkle();
                if (!!this.rightMoveAudio)
                    Util.playSfx(this.rightMoveAudio);
            ...
            .call(() => {
                if (this.moved && needToLog) {
                    if (!!this.wrongMoveAudio)
                        Util.playSfx(this.wrongMoveAudio);
    ...
    private moveToPos(pos: Vec2): cc.Tween {
        return new cc.Tween().target(this.node)
            .to(0.25, { position: this.node.position, scaleX: this.checkRTLAndScale(1), scaleY: SCALE }, { progress: null, easing: 'elasticOut' })
