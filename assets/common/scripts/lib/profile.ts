  ...
  updateLessonProgress(
    ...
    if (cpm) {
      ...
      if (lessonId == config.course.id + "_PreQuiz") {
        const quizChapter = config.course.chapters.find(
          (c) => c.id == config.course.id + "_quiz"
        );
        if (quizChapter) {
          let currentCourse = config.course.chapters.find(
            (c) => c.id != config.course.id + "_quiz"
          );
          let qzId = 0;
          for (let index = 0; index + 2 < quizScores.length; index += 3) {
            if (
              ...
              currentCourse = config.course.chapters.find(
                (c) => c.id == config.course.levels[qzId]
              );
            } else {
              break;
            }
          ...
          cpm.updateChapterId(
            chapters[Math.floor((chapters.length - 1) * total)].id
          );
        }
      } else {
        if (Config.i.lesson.type != EXAM || score >= MIN_PASS) {
    ...
    let user = new User(
      ...
      debug
        ? new Map([
            ...
            ["sl_en1_mp", new CourseProgressClass("sl_en1_mp00")],
            ["sl_en2_mp", new CourseProgressClass("sl_en2_mp00")],
            ["sl_maths", new CourseProgressClass("sl_maths01")],
            ["sl_maths1_mp", new CourseProgressClass("sl_maths1_mp01")],
            ["sl_maths2_mp", new CourseProgressClass("sl_maths2_mp00")],
