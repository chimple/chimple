...
export class Util {
  ...
  public static loadDirectLessonWithLink(data: MicroLink, node: cc.Node) {
    ...
    config.loadSingleCourseJson(data.courseid, () => {
      ...
      config.chapter = config.course.chapters.find(
  ...
  public static changeHotUpdateServer(serverUrl: string): void {
    try {
      ...
      cc.log(
        "manifestJson.packageUrl",
        manifestJson.packageUrl,
        "manifestJson.remoteManifestUrl",
        manifestJson.remoteManifestUrl,
        "manifestJson.remoteVersionUrl",
        manifestJson.remoteVersionUrl
      );
      ...
      const success = jsb.fileUtils.writeStringToFile(
