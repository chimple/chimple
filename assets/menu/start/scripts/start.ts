...
import Config, {
  ...
  COURSES_LANG_ID,
  StartAction,
} from "../../../common/scripts/lib/config";
import {
  EXAM,
...
import TeacherAddedDialog, {
  TEACHER_ADD_DIALOG_CLOSED,
} from "../../../common/scripts/teacherAddedDialog";
import {
...
export default class Start extends cc.Component {
    ...
    Util.loadFriend((node: cc.Node) => {
      ...
      switch (startAction) {
        ...
        case StartAction.MoveLessonPlan:
          friendComp.helpFile =
            COMPLETE_AUDIOS[Math.floor(Math.random() * COMPLETE_AUDIOS.length)];
        ...
        case StartAction.Default:
          friendComp.helpFile =
            DEFAULT_AUDIOS[Math.floor(Math.random() * DEFAULT_AUDIOS.length)];
          break;
      }
  ...
  private showReConnectPopup(
    msg: string,
    title: string = "Do you want to connect now?"
  ) {
    const reConnectPopupNode = this.node.getChildByName("reconnect_popup");
    if (reConnectPopupNode.active == false) {
      reConnectPopupNode
        .getComponent(ReConnectPopup)
        .msg.getComponent(ChimpleLabel).string = Util.i18NText(msg);
      reConnectPopupNode
        .getComponent(ReConnectPopup)
        .text.getComponent(ChimpleLabel).string = Util.i18NText(title);
  ...
  private getNextCourse() {
    ...
    if (User.getCurrentUser().isConnected && mode != Mode.School) {
      if (
        this.isAssignmentsExistsInLessonPlan() ||
        (!!Config.i.getAssignmentLessonsTodo() &&
          Config.i.getAssignmentLessonsTodo().length > 0)
      ) {
        return Config.i.curriculum.get(ASSIGNMENT_COURSE_ID);
      }
      return Config.i.curriculum.get(
        ar[0] === ASSIGNMENT_COURSE_ID ? ar[1] : ar[0]
      );
    } else {
      return Config.i.curriculum.get(
        ar[1] === ASSIGNMENT_COURSE_ID ? ar[0] : ar[1]
      );
    }
  }
    ...
    if (Config.isMicroLink && data && data.length > 0) {
      ...
      if (cc.sys.isNative) {
        if (COURSES_LANG_ID.includes(courseDetails["courseid"])) {
          const courseProgress = user.courseProgressMap.get(
  ...
  private showTeacherDialog() {
    try {
      const messageStr: string =
        cc.sys.localStorage.getItem(RECEIVED_TEACHER_REQUEST) || "[]";
      ...
      if (messages && messages.length > 0) {
        ...
        const teacherRequestsAccepted = JSON.parse(
          cc.sys.localStorage.getItem(tKey) || "[]"
        );
        const buildLink = id + "|" + sectionId + "|" + addStudentId;
        const linkUsed: boolean = teacherRequestsAccepted.includes(buildLink);
        cc.log(
          `checking if received teacher request link ${buildLink}  is used ${linkUsed}`
        );
        if (!!id && !!name && !!sectionId && !!addStudentId && !linkUsed) {
          const teacherDialog: cc.Node = cc.instantiate(
            this.teacherDialogPrefab
          );
          const script: TeacherAddedDialog =
            teacherDialog.getComponent(TeacherAddedDialog);
  ...
  private createLessonPlan(courseId: string) {
    ...
    if (courseId == ASSIGNMENT_COURSE_ID) {
      ...
      if (lessonPlan != null && lessonPlan.length > 0) {
        courseProgress.lessonPlan = lessonPlan
          .slice(0, Math.min(5, lessonPlan.length))
          .map((les) => les.id);
      ...
      const currentChapter = course.chapters.find(
        (chapter: Chapter) => chapter.id == courseProgress.currentChapterId
      );
      if (
        currentChapter &&
        (!courseProgress.currentLessonId ||
          !currentChapter.lessons.find(
            (l) => l.id == courseProgress.currentLessonId
          ))
      ) {
      ...
      if (course.id == "puzzle") {
        ...
        course.chapters.forEach((ch) => {
          let puzLes: Lesson;
          puzLes = ch.lessons.find(
            (l, i, ls) => !user.lessonProgressMap.has(l.id)
          );
          if (!puzLes) {
            const randomInt = Math.floor(
              Math.random() * (ch.lessons.length - 1 - 0 + 1) + 0
            );
        ...
        if (!courseProgress.currentChapterId) {
          ...
          lessons = this.getLessonsForPlan(
            currentChapter,
            courseProgress.currentLessonId
          );
  ...
  checkPendingAssignments() {
    ...
    for (let assign of Config.i.assignments) {
      ...
      if (!!lesson) {
        const lessonProgress: LessonProgress =
          User.getCurrentUser().lessonProgressMap.get(assign.lessonId);
        if (!lessonProgress) {
          count++;
          ...
          !!lessonProgress &&
          ![].concat(lessonProgress.assignmentIds).includes(assign.assignmentId)
        ) {
    ...
    this.assignmentCount.getChildByName("count").getComponent(cc.Label).string =
      count.toString();
  }
  ...
  displayLessonPlan() {
    ...
    if (
      courseProgressMap.lessonPlan != null &&
      courseProgressMap.lessonPlan.length > 0
    ) {
      ...
      courseProgressMap.lessonPlan.forEach((lessonId, index, lessons) => {
        const node: cc.Node = Start.createLessonButton(
          lessonId.endsWith("_PreQuiz")
            ? Start.preQuizLesson(
                Config.i.curriculum.get(lessonId.split("_")[0])
              )
        ...
        if (index == courseProgressMap.lessonPlanIndex) {
          ...
          if (Config.i.startAction == StartAction.MoveLessonPlan && index > 0) {
            ...
            currentLessonNode.position = node.convertToNodeSpaceAR(
              prevNode.convertToWorldSpaceAR(cc.v3(0, 300, 0))
            );
            currentLessonNode.runAction(
              cc.bezierTo(1, [
      ...
      this.gift.x =
        Math.pow(1 - 1, 3) * x1 +
        3 * Math.pow(1 - 1, 2) * 1 * x2 +
        3 * (1 - 1) * Math.pow(1, 2) * x3 +
        Math.pow(1, 3) * x4;
      this.gift.y =
        Math.pow(1 - 1, 3) * y1 +
        3 * Math.pow(1 - 1, 2) * 1 * y2 +
        3 * (1 - 1) * Math.pow(1, 2) * y3 +
        Math.pow(1, 3) * y4;
      // this.node.getChildByName('giftBox').addChild(this.gift)
      if (
        this.node.getChildByName("giftBox").getChildByName(this.gift.name) ==
        undefined
      ) {
        this.node.getChildByName("giftBox").addChild(this.gift);
        // this.displayCurrentReward();
  ...
  private giveReward(user: User) {
    console.log(
      "giveReward node",
      this.node.getChildByName("giftBox").children[0]
    );
    let node: cc.Node = this.node.getChildByName("giftBox").children[0];
    console.log("giveReward node", node);
    var seq = cc.repeat(
      cc.sequence(cc.scaleTo(0.3, 1.2, 1.2), cc.scaleTo(0.3, 1, 1)),
      100
    );
    node.runAction(seq);
    this.node.getChildByName("beginQuizPopup").active = false;
  ...
  public static createPreQuizButton(
    course: Course,
    lessonButtonPrefab: cc.Prefab,
    loading: cc.Node,
    open: boolean
  ): cc.Node {
    const lesson = {
  ...
  public static createLessonButton(
    lesson: Lesson,
    lessonButtonPrefab: cc.Prefab,
    loading: cc.Node,
    open: boolean
  ...
  async showAssignmentPopup(pendingAssignment: boolean) {
    if (this.flag && !Config.isMicroLink) {
      this.flag = false;
      const user = User.getCurrentUser();
      this.assignments = await ServiceConfig.getI().handle.listAssignments(
        user.id
      );
      ...
      if (this.assignments.length > 0 && !pendingAssignment) {
        ...
        if (this.previousHash != currentHash) {
          ...
          const assignmentPopupNode =
            this.node.getChildByName("assignment_popup");
          if (assignmentPopupNode.active == false) {
            assignmentPopupNode
              .getComponent(AssignmentPopup)
              .msg.getComponent(ChimpleLabel).string =
              "New assignment has been assigned to you";
  ...
  private getNextReward(): string[] {
    ...
    const isRewardUnlocked = (reward: string): boolean =>
      !!User.getCurrentUser().unlockedRewards[reward];
    for (const chapter of course.chapters) {
      for (const lesson of chapter.lessons) {
        if (
          !User.getCurrentUser().unlockedRewards[
            `${REWARD_TYPES[4]}-${chapter.id}-${lesson.id}`
          ]
        )
        ...
        if (!!lesson?.skills) {
          for (const single of lesson.skills) {
            if (
              !User.getCurrentUser().unlockedRewards[
                `${REWARD_TYPES[4]}-${chapter.id}-${lesson.id}-${single}`
              ]
            )
    ...
    if (this.gift) {
      ...
      switch (currentReward[0]) {
        case REWARD_TYPES[0]: //character
          cc.resources.load(
            `char_icons/${currentReward[1]}_icon`,
            (err, sp) => {
        ...
        case REWARD_TYPES[1]: //background
          cc.resources.load(
            `backgrounds/textures/bg_icons/background-${currentReward[1]}`,
            (err, sp) => {
              const image = new cc.Node();
              const imageComp = image.addComponent(cc.Sprite);
              // @ts-ignore
        ...
        case REWARD_TYPES[3]: //inventory
          cc.resources.load(
            INVENTORY_ICONS[currentReward[2]] + currentReward[3],
            (err, sp) => {
              if (err) {
        ...
        case REWARD_TYPES[4]: //lesson
          if (currentReward[1] == "sticker") {
            Config.loadBundle(
              ...
              (bundle) => {
                bundle.load(
                  `res/${currentReward[1]}-${currentReward[2]}`,
                  cc.Texture2D,
                  (err, asset) => {
                    if (err) {
                      cc.log(JSON.stringify(err));
            ...
            Util.load(
              "reward/course/res/icons/" + lesson.image,
              (err, texture) => {
                if (!err) {
  ...
  toAddGiftBoxNode(image: cc.Node, type) {
    ...
    let sprite: cc.SpriteFrame = new cc.SpriteFrame(type),
      width: number = 64,
      height: number = 64,
      max: number = 1;
    const { scale, size } = Util.minScale(imageComp, width, height, max);
    console.log(
      "const size = sprite.getOriginalSize();",
      imageComp.spriteFrame.getOriginalSize(),
      size,
      "scale",
      scale
    );
    ...
    if (
      ...
      this.gift.y =
        ...
        Math.pow(1, 3) * y4;
      this.node
        .getChildByName("giftBox")
        .children[0].getChildByName(image.name).width = scale * size.width;
      this.node
        .getChildByName("giftBox")
        .children[0].getChildByName(image.name).height = scale * size.height;
    } else {
      if (
        this.node
          .getChildByName("giftBox")
          .children[0].getChildByName(image.name) != undefined
      ) {
        // @ts-ignore
        this.node
          .getChildByName("giftBox")
          .children[0].getChildByName(image.name)
          .getComponent(cc.Sprite).spriteFrame = new cc.SpriteFrame(type);
        this.node
          .getChildByName("giftBox")
          .children[0].getChildByName(image.name).width = scale * size.width;
        this.node
    ...
    if (
      !this.disableGiftBoxNodeFlag &&
      user.isConnected &&
      Config.i.course.id == ASSIGNMENT_COURSE_ID &&
      (courseProgressMap.lessonPlan == null ||
        courseProgressMap.lessonPlan.length <= 0)
    ) {
      ...
      User.getCurrentUser().isConnected &&
      Config.i.course.id != ASSIGNMENT_COURSE_ID
    ) {
      console.log("if called");
      this.disableGiftBoxNodeFlag = true;
