...
export default class WelcomePage extends cc.Component {
  ...
  onLoad() {
    try {
      ...
      Util.loadi18NMapping(() => {
        const lang = Lang.SIERRA_LEONE_ENGLISH;
  ...
  //       languageButton.getComponentInChildren(cc.Label).string =
  //         LANG_CONFIGS.get(ALL_LANGS[ALL_LANGS.indexOf(data)]).displayName;
  ...
  selectModes() {
    ...
    switch (modes) {
      case Mode.School:
        ...
        Config.i.pushScene(
          "private/school/scenes/selectSections",
          "private",
          null,
          true
        );
  ...
  layoutManager() {
    try {
      ...
      filteredUsers.forEach((e) => {
        cc.log(e);
        let userButtonRef;
        if (!!this.userButton) userButtonRef = cc.instantiate(this.userButton);
        if (!!userButtonRef) {
          userButtonRef.getChildByName("Label").getComponent(cc.Label).string =
            e.name;
          // LOAD AVATAR
            ...
            try {
              if (!err) {
                ...
                userButtonRef
                  ...
                  .getChildByName("icon")
                  .getComponent(cc.Sprite).spriteFrame = new cc.SpriteFrame(sp);
              }
            } catch (error) {
