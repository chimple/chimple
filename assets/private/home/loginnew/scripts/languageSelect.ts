...
export default class LanguageSelect extends cc.Component {
  ...
  onLoad() {
    Profile.setValue(LANGUAGE, "sl_en");
    ...
    LANG_CONFIGS.forEach((ele, i) => {
      ...
      languagePrefab.children[0].getChildByName("Background").color =
        bgColor.fromHEX(`${ele.colorCode}`);
      languagePrefab
        .getChildByName("namesymbol")
        .getComponent(cc.Label).string = ele.symbol;
      languagePrefab.getChildByName("namelabel").getComponent(cc.Label).string =
        ele.displayName;
      if (i === "sl_en") {
  ...
  onLanguageClick(event, customEventData) {
    for (let i = 0; i < this.languageContainerNode.childrenCount; i++) {
      this.languageContainerNode.children[i].getChildByName(
        "ticksprite"
      ).active = false;
    }
    ...
    if (Profile.lang != customEventData) {
      ...
      Util.loadi18NMapping(() => {
        const chimpleLabelComponent =
          this.questionNode.getComponent(ChimpleLabel);
        if (chimpleLabelComponent) {
          chimpleLabelComponent.string = Util.i18NText(
            chimpleLabelComponent.key
          );
        }
      });
    }
